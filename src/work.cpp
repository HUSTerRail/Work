...
bool find(int target, vector<bool>& used, vector<int>& match, vector<int>& w,map<int, vector<int>>&graph){
    vector<int> neighbors = graph[target];
    for(int i = 0; i <neighbors.size(); i++){ //遍历每个偶数与奇数比较
            if(w[neighbors[i]] == w[target] && used[i] ==0){
                 used[i] = true;
                if(match[neighbors[i]] == 0 || find(match[neighbors[i]], used, match,w,graph)){ 
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        if(find(i, visited, match, w, graph)){
            result++;
        }
